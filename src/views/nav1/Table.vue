...
//		            '</el-table>',
         template:'<div><el-input v-model="getText" width="600"></el-input><el-popover ref="popBtn" placement="right" width="500" trigger="click">'+
//         				'<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">'+
         '<el-form :inline="true" :model="filters">'+
        	 '<el-form-item>'+
...
//				}]
        ...
        methods: {
            handleCurrentChange:function(currentRow,oldCurrentRow){
				this.value = currentRow[popConfig[this.popKey].value];
                this.name = currentRow[popConfig[this.popKey].name];
                this.$emit('changeValue',[this.value,this.name]);
			},
            getData:function(){
